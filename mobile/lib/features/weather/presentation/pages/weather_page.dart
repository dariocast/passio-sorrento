...
class WeatherPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      )..loadWeather(AppConstants.municipalities.first),
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            isScrollable: true,
            tabs: AppConstants.municipalities
                .map((m) => Tab(text: m))
                .toList(),
            onTap: (index) {
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () => context.read<WeatherCubit>().loadWeather(
                              state.selectedMunicipality ?? AppConstants.municipalities.first,
                            ),
                  ...
                  child: Column(
                    children: [
                      ...
                      if (state.forecast.isNotEmpty) ...[
                        ...
                        const SizedBox(height: 8),
                        ...state.forecast.take(5).map((f) => Card(
                              child: ListTile(
                                leading: Icon(_weatherIcon(f.icon)),
                                title: Text('${f.temperature.round()}°C - ${f.description}'),
                                subtitle: Text(_formatDateTime(f.timestamp)),
                                trailing: Text('${f.precipitationProbability}%'),
                              ),
                            )),
                      ],
                    ],
